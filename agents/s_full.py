...
def run_subagent(prompt: str, agent_type: str = "Explore") -> str:
    ...
    for _ in range(30):
        resp = LLM.create(
            model=MODEL,
            messages=sub_msgs,
            tools=convert_tools_to_openai_format(sub_tools),
            max_tokens=8000,
        )
        sub_msgs.append({"role": "assistant", "content": get_response_content(resp)})
        if not LLM.is_tool_call(resp):
            break
        results = []
        for b in LLM.get_tool_calls(resp):
            h = sub_handlers.get(LLM.get_tool_name(b), lambda **kw: "Unknown tool")
            results.append(
                LLM.format_tool_result(b, str(h(**LLM.get_tool_args(b)))[:50000])
            )
        sub_msgs.append({"role": "user", "content": results})
    if resp:
        return get_response_text(resp) or "(no summary)"
...
def get_response_content(response) -> str:
    return LLM.get_response_text(response)
...
def auto_compact(messages: list) -> list:
    ...
    summary = get_response_text(resp)
    return [
        ...
        while True:
            ...
            for _ in range(50):
                ...
                try:
                    ...
                    return
                messages.append(
                    {"role": "assistant", "content": get_response_content(response)}
                )
                ...
                for block in LLM.get_tool_calls(response):
                    block_name = LLM.get_tool_name(block)
                    block_args = LLM.get_tool_args(block)
                    if block_name == "idle":
                        idle_requested = True
                        output = "Entering idle phase."
                    elif block_name == "claim_task":
                        output = self.task_mgr.claim(block_args["task_id"], name)
                    elif block_name == "send_message":
                        output = self.bus.send(
                            name, block_args["to"], block_args["content"]
                        )
                    else:
                        dispatch = {
                            "bash": lambda **kw: run_bash(kw["command"]),
                        ...
                        output = dispatch.get(block_name, lambda **kw: "Unknown")(
                            **block_args
                        )
...
def agent_loop(messages: list):
    ...
    while True:
        ...
        for block in LLM.get_tool_calls(response):
            block_name = LLM.get_tool_name(block)
            block_args = LLM.get_tool_args(block)
            if block_name == "compress":
                manual_compress = True
            handler = TOOL_HANDLERS.get(block_name)
            try:
                output = (
                    handler(**block_args) if handler else f"Unknown tool: {block_name}"
                )
            except Exception as e:
                output = f"Error: {e}"
            print(f"> {block_name}: {str(output)[:200]}")
            results.append(LLM.format_tool_result(block, str(output)))
            if block_name == "TodoWrite":
                used_todo = True
